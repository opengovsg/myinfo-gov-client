...
class MyInfoGovClient {
  ...
  /**
   *  Verifies if the basestring was signed with private key that corresponds
   *  to provided public key.
   *  @param {Object} params - Object containing fields to verify
   *  @param {string} params.basestring - Basestring constructed from
   *  _formulateBaseString()
   *  @param {string} params.signature - Signature generated by
   *  _signBaseString()
   *  @param {string} params.publicKey - Public key corresponding to private
   *  key used for signature generation
   *  @return {boolean} - True if signature is verified, false otherwise.
   */
  verifySignature ({ basestring, signature, publicKey }) {
